...
type ReceiptLine = { itemId: number; locationId: number; qty: number }
type Receipt = { id: number; number: string; supplier?: string | null; status: 'draft' | 'posted' | string; lines: ReceiptLine[] }
...
export default function Primke() {
  ...
  function onEdit() {
    ...
    if (!r) return
    if (r.status !== 'draft') {
      setToast({ open: true, sev: 'error', msg: t('receipts.onlyDraftEditable') })
      return
    }
    setEditingId(r.id)
    setNumber(r.number)
  ...
  async function onDelete() {
    ...
    if (postedCount > 0) {
      const proceed = window.confirm(t('receipts.confirm.deletePostedWarning', { count: postedCount }))
      if (!proceed) return
    }
    const ok = window.confirm(selected.length === 1 ? t('receipts.confirm.deleteOne') : t('receipts.confirm.deleteMany', { count: selected.length }))
    if (!ok) return
    try {
      setSaving(true)
      const ids = [...selected]
      const results = await Promise.allSettled(ids.map(id => api.receipts.delete(id)))
      ...
      if (failed) {
        ...
        setToast({ open: true, sev: 'success', msg: t('receipts.toast.deleted') })
      }
    } catch (e: any) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!number.trim()) return setToast({ open: true, sev: 'error', msg: t('receipts.enterNumber') })
    if (lines.length === 0) return setToast({ open: true, sev: 'error', msg: t('receipts.atLeastOneLine') })
    ...
    try {
      ...
      if (editingId == null) {
        ...
        setToast({ open: true, sev: 'success', msg: t('receipts.saved') })
      } else {
        const updated = await api.receipts.update(editingId, { number, supplier, lines })
        setRows(prev => prev.map(r => (r.id === editingId ? updated : r)))
        setToast({ open: true, sev: 'success', msg: t('receipts.updated') })
      }
      setDialogOpen(false)
      setEditingId(null)
    } catch (e: any) {
  ...
  async function post(id: number) {
    try {
      ...
      setToast({ open: true, sev: 'success', msg: t('receipts.posted') })
  ...
  return (
    <Box sx={{ display: 'grid', gap: 3 }}>
      <Typography variant="h4" fontWeight={700}>{t('receipts.title')}</Typography>
              ...
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    ...
                    inputProps={{ 'aria-label': t('receipts.aria.selectAll') as string }}
                  />
                </TableCell>
                <TableCell>{t('common.number')}</TableCell>
                <TableCell>{t('receipts.supplier')}</TableCell>
              ...
              {rows.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ color: 'text.secondary' }}>
                    {loading ? t('common.loading') : t('receipts.empty')}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="md">
        <DialogTitle>{editingId == null ? t('receipts.dialog.newTitle') : t('receipts.dialog.editTitle')}</DialogTitle>
        <Box component="form" onSubmit={onSubmit}>
          <DialogContent sx={{ pt: 1 }}>
            <Stack spacing={2}>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <TextField label={t('receipts.form.number')} value={number} onChange={e => setNumber(e.target.value)} fullWidth />
                <TextField label={t('receipts.form.supplier')} value={supplier} onChange={e => setSupplier(e.target.value)} fullWidth />
              </Stack>
              ...
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                <Autocomplete
                  ...
                  renderInput={(p) => <TextField {...p} label={t('receipts.form.item')} />}
                  sx={{ minWidth: 280, flex: 1 }}
                />
                <Autocomplete
                  options={locations}
                  getOptionLabel={(o) => o.code}
                  value={locations.find(l => l.id === line.locationId) || null}
                  onChange={(_, v) => setLine(l => ({ ...l, locationId: v ? v.id : null }))}
                  renderInput={(p) => <TextField {...p} label={t('receipts.form.location')} />}
                  sx={{ minWidth: 220, flex: 1 }}
                />
                <TextField
                  type="number"
                  label={t('receipts.form.qty')}
                  value={line.qty}
          ...
          <DialogActions>
            ...
            <Button type="submit" variant="contained" startIcon={<SaveIcon />} disabled={saving}>
              {editingId == null ? t('receipts.actions.save') : t('receipts.actions.saveChanges')}
            </Button>
          </DialogActions>
        </Box>
      </Dialog>
