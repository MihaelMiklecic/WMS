...
type Dispatch = { id: number; number: string; customer?: string | null; status: string; lines: any[] }
...
export default function Otpremnice() {
  ...
  React.useEffect(() => {
    ;(async () => {
      try {
        const [docs, its, locs] = await Promise.all([api.dispatches.list(), api.items.list(), api.locations.list()])
  ...
  async function create() {
    if (!number.trim()) return setToast({ open: true, sev: 'error', msg: 'Unesite broj otpremnice' })
    ...
    try {
      ...
      setCustomer('')
      setToast({ open: true, sev: 'success', msg: 'Otpremnica spremljena' })
    } catch (e: any) {
  ...
  async function post(id: number) {
    try {
      ...
      setToast({ open: true, sev: 'success', msg: 'Otpremnica proknjižena' })
    } catch (e: any) {
  ...
  return (
    <Box sx={{ display: 'grid', gap: 3 }}>
      <Typography variant="h4" fontWeight={700}>Otpremnice</Typography>

      <Paper variant="outlined" sx={{ p: 2, maxWidth: 1000 }}>
        <Stack spacing={2}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <TextField label="Broj otpremnice" value={number} onChange={e => setNumber(e.target.value)} fullWidth />
            <TextField label="Kupac" value={customer} onChange={e => setCustomer(e.target.value)} fullWidth />
          </Stack>
        ...
        <Paper variant="outlined">
          <Table size="small">
            ...
            <TableBody>
              ...
              {rows.length === 0 && (
                <TableRow><TableCell colSpan={5} align="center" sx={{ color: 'text.secondary' }}>Još nema otpremnica.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </Paper>
